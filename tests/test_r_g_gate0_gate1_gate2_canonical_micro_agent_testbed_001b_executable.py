...
from r_g_gate0_gate1_gate2_canonical_micro_agent_testbed_001b.runner import (  # noqa: E402
    run_preflight_001b,
)
...
ARTIFACT_DIR = ROOT / "artifacts" / "r_g_gate0_gate1_gate2_canonical_micro_agent_testbed_001b"
TASK_CARD = (
    ROOT
    / "docs"
    / "codex"
    / "tasks"
    / "R-G-GATE0-GATE1-GATE2-CANONICAL-MICRO-AGENT-TESTBED-EXECUTABLE-PREFLIGHT-001B.md"
)
PARENT_TASK_CARD = (
    ROOT
    / "docs"
    / "codex"
    / "tasks"
    / "R-G-GATE0-GATE1-GATE2-CANONICAL-MICRO-AGENT-TESTBED-TASK-CARD-001A.md"
)
...
REQUIRED_TRACE_FIELDS = {
    ...
    "gate2_to_later_action_linkage_key",
...
def test_stage0_freeze_manifest_anchors_parent_commits_and_contract(tmp_path):
    ...
    assert "gate0_gate1_gate2" in stage0["shared_state_schema_id"]
...
def test_trace_rows_have_required_loop_fields_and_hash_chain(tmp_path):
    run_preflight_001b(repo_root=ROOT, output_dir=tmp_path)
    ...
    schema_ids = {row["shared_state_schema_id"] for row in rows}
    run_ids = {row["micro_agent_run_id"] for row in rows}
...
def test_shared_state_trace_uses_one_schema_and_records_gate_lineage(tmp_path):
    run_preflight_001b(repo_root=ROOT, output_dir=tmp_path)

    rows = _read_jsonl(tmp_path / "shared_state_trace.jsonl")
    assert len(rows) >= 8
    assert {row["shared_state_schema_id"] for row in rows} == {
        "canonical_gate0_gate1_gate2_micro_agent_shared_state_v1"
    }
    ...
    assert any(row["gate0_update_applied"] for row in rows)
    assert any(row["gate1_replay_or_consolidation_applied"] for row in rows)
    assert any(row["gate2_boundary_update_applied"] for row in rows)
    assert all(row["later_action_depends_on_shared_state_after_gate2"] for row in rows)


def test_linkage_keys_are_deterministic_label_free_and_collision_free(tmp_path):
    run_preflight_001b(repo_root=ROOT, output_dir=tmp_path)
    ...
    assert set(report["forbidden_label_tokens"]) == {
        "target",
        "oracle",
        "label",
        "future_outcome",
    ...
    serialized = json.dumps(report["linkage_derivation_inputs"]).lower()
    assert "future_outcome" not in serialized
    assert "expected_later_action" not in serialized
...
def test_baseline_comparison_includes_required_families_and_blocks_stitching(tmp_path):
    ...
    assert report["candidate"]["integrated_shared_state_score"] == 1.0
    assert report["acceptance_threshold"] == 0.95
    assert report["baseline_gate_passed"] is True
    assert report["best_fair_baseline"]["integrated_shared_state_score"] < report["acceptance_threshold"]
    assert baselines["stitched-output baseline with no shared state"][
        "integrated_shared_state_score"
    ] < report["candidate"]["integrated_shared_state_score"]
    assert baselines["stitched-output baseline with no shared state"][
        "uses_single_canonical_shared_state"
    ] is False
    assert baselines["oracle environment-label control as upper-bound/leakage only"][
        "counts_as_fair_baseline"
    ] is False
    assert baselines["trace-only replay as hygiene only"]["counts_as_fair_baseline"] is False
    assert report["strongest_baseline_explanation"]
...
def test_required_ablations_are_all_sensitive(tmp_path):
    ...
    assert all(row["integrated_shared_state_score"] < report["candidate_score"] for row in ablations.values())
    assert report["most_damaging_ablation"]["ablation_name"] in REQUIRED_ABLATIONS
...
def test_leakage_replay_and_mutation_reports_pass(tmp_path):
    ...
    assert leakage["hidden_labels_available_to_candidate"] is False
...
def test_result_preserves_claim_ceiling_and_anti_sycophancy_audit(tmp_path):
    ...
    assert result["claim_ceiling"] == CLAIM_CEILING
    assert result["verdict"] == VERDICT
    assert result["layer"] == "bounded canonical micro-agent integration executable preflight only"
    ...
    assert audit["tests_mechanism_or_behavioral_resemblance"] == (
        "bounded integrated proxy behavior with trace/replay/intervention sensitivity, "
        "not mechanism validity"
    )
