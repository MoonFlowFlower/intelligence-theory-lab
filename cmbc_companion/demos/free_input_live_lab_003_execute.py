...
def base_result(input_rows: list[dict[str, Any]], input_path: str | Path | None) -> dict[str, Any]:
    ...
    return {
        ...
        "outcome_coding": {
            "outcome_coding_ledger_count": 0,
            "feedback_encoded_as_outcome": False,
            "raw_text_memory_only": False,
            "outcome_coding_stable": False,
            "not_applicable_reason": None if input_available else "no_free_input_turns_to_code",
        },
        "causal_probe_extraction": {
            ...
            "formed_probe_types": [],
            "failure_reason": None if stable_probes_formed else "free_input_cannot_form_stable_causal_probes",
        },
        "metrics": {
            "free_input_turn_count": free_input_turn_count,
            "causal_probe_case_count": 0,
        ...
        "claim_after_execution": CLAIM_NO_INPUT if not input_available else "inconclusive free-input execution requires scoring",
        "not_authorized": [
            "EGO migration",
...
def write_artifacts(out_path: Path, result: dict[str, Any], input_rows: list[dict[str, Any]]) -> None:
    ...
    (out_path / "outcome_coding_ledger.jsonl").write_text("", encoding="utf-8")
