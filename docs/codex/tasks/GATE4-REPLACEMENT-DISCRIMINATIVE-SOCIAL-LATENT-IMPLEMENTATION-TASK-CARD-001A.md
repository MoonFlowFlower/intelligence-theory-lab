# GATE4-REPLACEMENT-DISCRIMINATIVE-SOCIAL-LATENT-IMPLEMENTATION-TASK-CARD-001A
...
> Implement replacement Gate4 so it passes.
...
Forbidden upgrade: claiming that this proxy proves agency, selfhood,
consciousness, real emotion, relationship learning, stable autonomy,
functional subject success, Gate5 readiness, admission readiness, runtime
readiness, bridge readiness, or EGO-mainline readiness.

## Prior Negative Evidence To Inherit

This future card must cite and preserve the following negative-evidence chain:

- `docs/research/GATE4-001C-NEGATIVE-EVIDENCE-ROUTING-RECORD-001A.md`
  - Gate4 001C is closed as non-discriminative negative evidence.
  - Future Gate4 work must be a new discriminative formulation, not a 001C
    repair.
...
- `remote-anchor-gate4-001c-negative-evidence-routing-record-001a-5a25838`
  at `5a2583869f8bf3ede40ad447434fd3e9b759b803`
- `remote-anchor-gate4-001c-execution-repair-rerun-001e-negative-evidence-2b167fb`
  at `2b167fb8ca5b9e942e313647d11bd337cc6fadf8`
...
Task-card drafting only. No code or experiment is authorized in this task.
...
## Current-Track Best Solution

Under the current track, the best next executable unit is not full Gate4
execution. It is a separate future implementation task that first creates the
minimal isolated harness, schemas, candidate proxy, baselines, ablations,
...
This drafting task creates no future implementation files. If a later task
explicitly invokes this card for implementation, the only allowed future
implementation paths are:

- `src/gate4_replacement_discriminative_social_latent_001b/__init__.py`
- `src/gate4_replacement_discriminative_social_latent_001b/__main__.py`
- `src/gate4_replacement_discriminative_social_latent_001b/schemas.py`
- `src/gate4_replacement_discriminative_social_latent_001b/episodes.py`
- `src/gate4_replacement_discriminative_social_latent_001b/candidate.py`
- `src/gate4_replacement_discriminative_social_latent_001b/baselines.py`
- `src/gate4_replacement_discriminative_social_latent_001b/ablations.py`
- `src/gate4_replacement_discriminative_social_latent_001b/leakage.py`
- `src/gate4_replacement_discriminative_social_latent_001b/replay.py`
- `src/gate4_replacement_discriminative_social_latent_001b/metrics.py`
- `src/gate4_replacement_discriminative_social_latent_001b/provenance.py`
- `src/gate4_replacement_discriminative_social_latent_001b/runner.py`
...
1. Schema and episode contract.
   - Define serializable observation, action, feedback, state, split, and
     provenance schemas.
   - Prohibit raw `seed`, `context_id`, `partner_id`, phase token, split label,
     oracle latent label, artifact path, bundle path, and answer-key fields from
     the legal decision interface.
2. Episode generation.
   - Generate only bounded offline toy episodes.
   - Include all mandatory split families:
     `seen_partner_unseen_context`, `unseen_partner_seen_context`,
     `unseen_partner_unseen_context`, `unseen_preference_transition`,
     `counterfactual_intervention`, `active_query_required_episodes`, and
     `heldout_episodes_with_remapped_ids`.
   - Predeclare at least 5 independent seeds.
3. Candidate proxy.
   - Implement a bounded stateful candidate with uncertainty, active query
     choice, feedback update, serialized state, and later-action dependence on
     updated state.
   - Do not use hidden labels unavailable at decision time.
4. Independent baselines.
   - Implement independent callable baselines:
     `stream_keyed_count_table`, `normalized_retrieval`,
     `nearest_neighbor_lookup`, `context_partner_table`, `graph_lookup`,
     `transition_table`, `successor_map`, `count_table`, `fsm_planner`,
     `episodic_traversal`, `no_state_ablation`, `no_feedback_ablation`,
     `no_action_ablation`, and `oracle_label_positive_control`.
   - Baselines may share schemas and metric utilities, but not candidate policy,
     update logic, latent inference, or state transition code.
5. Ablations.
   - Rerun episodes under real interventions:
     `no_state`, `no_feedback`, `no_action`, `no_active_query`,
     `shuffled_feedback`, and `counterfactual_transition`.
   - Ablations must not be post-hoc score edits.
6. Leakage scanner.
   - Scan the real output bundle and at least one deliberately contaminated
     positive-control bundle.
   - Include ID, phase-token, bundle-path, oracle-label, answer-key, and
     deterministic-order scans.
   - Excluded paths must be `not_scanned`, never `clean`.
7. Replay recomputation.
   - Recompute action and state update from `serialized_state`, observation,
     action policy path, update path, and feedback history.
...
9. Runner and artifacts.
   - Write machine-readable artifacts only under
     `artifacts/gate4_replacement_discriminative_social_latent_001b/`.
...
- Ties block positive evidence.
- Aggregate `candidate_minus_strongest_non_oracle_baseline >= 0.15`.
- Split-level `candidate_minus_strongest_non_oracle_baseline >= 0.10` on every
  critical split.
- No aggregate rescue: aggregate success cannot override any critical split
  failure.
- Confidence rule: lower bound of 95 percent interval must be greater than
  `0.05`, or the future task must use a predeclared repeated-seed stability rule
  and justify why intervals are not used.
- Repeated seed stability: at least 5 independent seeds; at least 4 of 5 must
...
For a later future implementation task:

1. Preserve failure artifacts under the future implementation artifact
   directory.
2. Do not tune thresholds after seeing failure.
3. Do not patch old artifacts into passes.
