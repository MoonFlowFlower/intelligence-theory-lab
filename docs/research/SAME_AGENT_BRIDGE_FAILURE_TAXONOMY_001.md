...
The claimed evidence is visible only in rendered behavior or UI output, not in
machine-readable trace/replay artifacts.

`oracle_leak`:
Hidden labels, future observations, seed identity, filenames, fixture names,
action names, or other leaked information can explain the result.

`threshold_tuning_dependency`:
The pass depends on thresholds selected or adjusted after observing outcomes.

`baseline_competence_missing`:
A mandatory baseline or control lacks a frozen competence check.

`ego_mainline_contamination`:
The task changed or depended on EGO mainline runtime, UI, companion behavior,
LLM integration, relationship learning, or deployment without explicit prior
authorization.

## Failure Handling

Failure labels are evidence, not embarrassment. Do not delete or rewrite failed
artifacts. A failed bridge can support a negative result or rollback decision,
but it cannot support a stronger mechanism claim.

If multiple labels apply, report all of them.

## What This Taxonomy Does Not Prove

This taxonomy does not classify consciousness, subjective experience, real
emotion, agency, stable autonomy, or functional subject success. Those claims
