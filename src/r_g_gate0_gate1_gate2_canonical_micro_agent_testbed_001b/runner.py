...
def _stage0_freeze_manifest(repo_root: Path) -> dict:
    task_card_sha = _file_sha(repo_root, core.EXECUTABLE_TASK_CARD_PATH)
    parent_card_sha = _file_sha(repo_root, core.PARENT_TASK_CARD_PATH)
    stage0 = {
        ...
        "parent_task_card_sha256": parent_card_sha,
        "executable_task_card_path": core.EXECUTABLE_TASK_CARD_PATH,
        "executable_task_card_sha256": task_card_sha,
...
def _execution_manifest(stage0: dict, freeze_manifest_hash: str, repo_root: Path) -> dict:
    source_paths = [
        ...
        "src/r_g_gate0_gate1_gate2_canonical_micro_agent_testbed_001b/__init__.py",
        "src/r_g_gate0_gate1_gate2_canonical_micro_agent_testbed_001b/__main__.py",
        "src/r_g_gate0_gate1_gate2_canonical_micro_agent_testbed_001b/core.py",
        "src/r_g_gate0_gate1_gate2_canonical_micro_agent_testbed_001b/runner.py",
...
def run_preflight_001b(
    ...
    trace_rows, shared_state_rows, later_predictions = core.build_candidate_run()
    _write_jsonl(out / "trace.jsonl", trace_rows)
    _write_jsonl(out / "shared_state_trace.jsonl", shared_state_rows)
    ledger.append(
        {
            "event": "candidate_shared_state_trace_written",
            "status": "written",
            "trace_rows": len(trace_rows),
            "shared_state_rows": len(shared_state_rows),
        }
    )
    ...
    replay = core.replay_integrity_report(trace_rows, shared_state_rows)
    ...
    _write_json(out / "later_behavior_evaluation.json", later_evaluation)
    _write_json(out / "later_action_evaluation.json", later_evaluation)
    ledger.append(
        {
            "event": "baseline_ablation_leakage_replay_reports_written",
            "status": "written",
            "best_fair_baseline_score": baseline["best_fair_baseline"][
