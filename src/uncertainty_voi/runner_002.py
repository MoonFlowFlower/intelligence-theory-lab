"""Isolated runner for UNCERTAINTY-VOI-REQUEST-MECHANISM-002A.
...
TASK_ID = "UNCERTAINTY-VOI-REQUEST-MECHANISM-002A"
EXPECTED_PREREG_SHA256 = "bc087cfc13102a57a624b7bb5d8684b50303db7c9be701bdd08d709bffffdf73"
CLAIM_CEILING = (
    ...
    "that beats both a tuned static rival and random-active rival on this specific "
    "hold-and-observe toy distribution."
)
VERDICT_SET = {
...
def _kg_component(mu_i: float, best_other: float, sigma_tilde: float) -> float:
    if sigma_tilde <= 0.0:
        return 0.0
    z = -abs(float(mu_i) - float(best_other)) / max(float(sigma_tilde), 1e-15)
    return float(sigma_tilde * (_normal_pdf(z) + z * _normal_cdf(z)))


def _predictive_update_std(var: np.ndarray, noise_sigma: float) -> np.ndarray:
    clipped = np.maximum(var, 0.0)
    return clipped / np.sqrt(clipped + float(noise_sigma) * float(noise_sigma))


def _query_kg_values(
    mu: np.ndarray,
    var: np.ndarray,
    cfg: FrozenConfig,
    *,
    ...
    sigma_tilde = _predictive_update_std(var, noise_sigma)
    if value_ablation:
        return sigma_tilde.copy()
    out = np.zeros(cfg.k, dtype=float)
    for i in range(cfg.k):
        others = np.delete(mu, i)
        best_other = float(np.max(others))
        out[i] = _kg_component(float(mu[i]), best_other, float(sigma_tilde[i]))
    return out
...
def choose_mechanism_action(
    ...
    *,
    ...
    query_kg = _query_kg_values(mu, var, cfg, noise_sigma=cfg.sigma_obs, value_ablation=value_ablation)
    query_scores = query_kg - cfg.query_cost
    top2 = np.argsort(mu)[-2:]
    request_kg_values = _query_kg_values(
        mu,
        var,
        cfg,
        noise_sigma=cfg.sigma_obs / 3.0,
        value_ablation=value_ablation,
    )
    request_kg = float(np.sum(request_kg_values[top2]))
...
def simulate_policy(
    ...
    *,
    ...
    regret = 0.0
    action_counts = {"noop": 0, "query": 0, "request": 0, "oracle": 0}
    trace_rows: list[dict[str, Any]] = []
    ...
    for t in range(cfg.horizon):
        ...
        elif policy_id == "random_active":
            action = {"kind": "query", "arm": int(rng.integers(0, cfg.k))}
        elif policy_id == "hold_only":
            action = {"kind": "noop"}
        ...
        incumbent_arm = int(np.argmax(pre_mu))
        cost = 0.0
        observations: list[dict[str, Any]] = []
        if action_kind == "oracle":
            regret_t = 0.0
        elif action_kind == "noop":
            regret_t = max_value - float(world.values[t, incumbent_arm])
        else:
            cost = cfg.query_cost if action_kind == "query" else cfg.request_cost
            regret_t = (max_value - float(world.values[t, incumbent_arm])) + cost
            ...
            trace_rows.append(
                {
                    ...
                    "incumbent_arm": int(incumbent_arm),
                    "information_cost": _round_float(cost),
                }
            )
...
def tune_static_threshold(cfg: FrozenConfig | None = None) -> dict[str, Any]:
    ...
    return {
        "producer_function": "uncertainty_voi.runner_002.tune_static_threshold",
...
def _condition_results(
    ...
    *,
    ...
    random_results = [
        simulate_policy("random_active", seed, cfg)
        for seed in seeds
    ]
    advantages_static = [s.regret - m.regret for m, s in zip(mech_results, static_results)]
    advantages_random = [r.regret - m.regret for m, r in zip(mech_results, random_results)]
    advantages_better = [
        min(s.regret, r.regret) - m.regret
        for m, s, r in zip(mech_results, static_results, random_results)
    ]
    return {
        ...
        "random_active_regrets": [float(r.regret) for r in random_results],
        "advantages_vs_static": [float(a) for a in advantages_static],
        "advantages_vs_random_active": [float(a) for a in advantages_random],
        "advantages_vs_better_rival": [float(a) for a in advantages_better],
        "advantage_static_stats": _paired_stats(advantages_static, cfg, label=f"{condition_policy_id}:static:{list(seeds)}"),
        "advantage_random_active_stats": _paired_stats(advantages_random, cfg, label=f"{condition_policy_id}:random_active:{list(seeds)}"),
        "advantage_better_rival_stats": _paired_stats(advantages_better, cfg, label=f"{condition_policy_id}:better_rival:{list(seeds)}"),
        "mechanism_action_counts": _sum_action_counts(mech_results),
        "static_action_counts": _sum_action_counts(static_results),
        "random_active_action_counts": _sum_action_counts(random_results),
...
def _floor_results(seeds: Sequence[int], cfg: FrozenConfig) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for policy_id in ("random_active", "hold_only", "no_update", "oracle"):
...
def replay_trace(trace_rows: Sequence[dict[str, Any]], cfg: FrozenConfig | None = None) -> dict[str, Any]:
    ...
    return {
        "producer_function": "uncertainty_voi.runner_002.replay_trace",
...
def run_rng_audit() -> dict[str, Any]:
    ...
    return {
        "producer_function": "uncertainty_voi.runner_002.run_rng_audit",
...
def _cost_projection(cfg: FrozenConfig, start_time: float, probe_elapsed: float) -> dict[str, Any]:
    ...
    return {
        "producer_function": "uncertainty_voi.runner_002._cost_projection",
...
def _compute_core_payload(cfg: FrozenConfig) -> dict[str, Any]:
    ...
    uncertainty_collapse = _paired_stats((clean_adv - uncertainty_adv).tolist(), cfg, label="collapse:uncertainty")
    ...
    mean_random = float(floors["random_active"]["mean_regret"])
    ...
    gates = {
        ...
        "G2_falsifiers": {
            "passed": bool(
                _ci_includes_zero(wrong["advantage_better_rival_stats"])
                and _ci_includes_zero(shuffled["advantage_better_rival_stats"])
        ...
        "G3_ablations": {
            "passed": bool(
                _ci_includes_zero(uncertainty["advantage_better_rival_stats"])
                and _ci_excludes_zero_positive(uncertainty_collapse)
                and _ci_excludes_zero_positive(value_collapse)
            ),
    ...
    verdict, subtype, positive_claim = _derive_verdict(gates, mean_mech, mean_random, mean_hold, mean_no_update)
    return {
        "stopped": False,
        "probe": {
            "seed": cfg.probe_seed,
...
def _derive_verdict(
    ...
    if (g1a and g1b) and (not g2 or not g3):
...
def _fresh_process_recompute(cfg: FrozenConfig, expected_digest: str) -> dict[str, Any]:
    ...
    return {
        "producer_function": "uncertainty_voi.runner_002._fresh_process_recompute",
...
def _build_artifacts_payload(cfg: FrozenConfig, core: dict[str, Any], replay_report: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        "task_id": TASK_ID,
        "producer_function": "uncertainty_voi.runner_002.run_gate",
    ...
    baseline_comparison = {
        "task_id": TASK_ID,
        "producer_function": "uncertainty_voi.runner_002._build_artifacts_payload/baseline_comparison",
    ...
    ablation_report = {
        "task_id": TASK_ID,
        "producer_function": "uncertainty_voi.runner_002._build_artifacts_payload/ablation_report",
        "code_path_hash": code_hash,
    ...
    falsifier_report = {
        "task_id": TASK_ID,
        "producer_function": "uncertainty_voi.runner_002._build_artifacts_payload/falsifier_report",
        "code_path_hash": code_hash,
    ...
    calibration_transfer_report = {
        "task_id": TASK_ID,
        "producer_function": "uncertainty_voi.runner_002._build_artifacts_payload/calibration_transfer_report",
        "code_path_hash": code_hash,
    ...
    headroom_report = {
        "task_id": TASK_ID,
        "producer_function": "uncertainty_voi.runner_002._build_artifacts_payload/headroom_report",
        "code_path_hash": code_hash,
    ...
    tripwire_report = {
        "task_id": TASK_ID,
        "producer_function": "uncertainty_voi.runner_002._build_artifacts_payload/tripwire_report",
        "code_path_hash": code_hash,
    ...
    metric_records = {
        "task_id": TASK_ID,
        "producer_function": "uncertainty_voi.runner_002._metric_records",
...
def _metric_records(
    ...
    records = [
        _build_metric_record(
            ...
            producer_function="uncertainty_voi.runner_002._condition_results",
            value={
        ...
        _build_metric_record(
            ...
            producer_function="uncertainty_voi.runner_002._derive_verdict",
        ...
        _build_metric_record(
            ...
            producer_function="uncertainty_voi.runner_002.sha256_lf",
            value=config_shas,
            input_artifacts=["preregistration.md", "runner_002.py"],
            seeds=[],
...
def _failure_manifest(core: dict[str, Any], replay_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "task_id": TASK_ID,
        "producer_function": "uncertainty_voi.runner_002._failure_manifest",
